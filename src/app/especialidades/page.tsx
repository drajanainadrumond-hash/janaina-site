          ...
          return (
            <div key={esp.num} className="reveal grid lg:grid-cols-2 gap-8 lg:gap-16 mb-14 lg:mb-20 items-center" style={reversed ? { direction: "rtl" } : undefined}>
              ...
              <div style={reversed ? { direction: "ltr" } : undefined}>
                <div className="font-heading text-[3rem] sm:text-[4rem] lg:text-[4.5rem] font-extralight text-cream-dark leading-none tracking-[2px]">{esp.num}</div>
                <div className="text-[1.125rem] uppercase tracking-[2.5px] text-teal-pale my-2">{esp.tag}</div>
                <h3 className="font-heading text-[1.3rem] sm:text-[1.5rem] lg:text-[1.8rem] font-normal tracking-[0.5px] mb-4 leading-[1.25]">{esp.title}</h3>
