    ...
    <div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-8 text-center border border-[#E6E5E2]">
          <p className="text-[1.125rem] text-[#7A8E9B]">{search ? "Nenhum resultado." : "Nenhum contato ainda."}</p>
        </div>
      ) : (
          ...
          {filtered.map((c) => (
            <div key={c.id} className="bg-white rounded-xl p-4 border border-[#E6E5E2] flex items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                ...
                {c.notes && <p className="mt-1 text-[1.125rem] text-[#7A8E9B] truncate">{c.notes}</p>}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                ...
                <button onClick={() => handleDelete(c.id)} className="px-3 py-1.5 rounded-lg text-[1.125rem] uppercase text-red-500 hover:bg-red-50 transition-colors">Excluir</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <p className="text-[1.125rem] text-[#7A8E9B] mt-4">{filtered.length} contato{filtered.length !== 1 ? "s" : ""}</p>
    </div>
  );
}
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={onCancel} className="text-[1.125rem] text-[#7A8E9B] hover:text-[#003E51]">Voltar</button>
      </div>

      <div className="bg-white rounded-2xl p-6 border border-[#E6E5E2] space-y-5">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[1.125rem] uppercase tracking-[1px] text-[#7A8E9B] mb-1.5">Nome</label>
          ...
          <div>
            <label className="block text-[1.125rem] uppercase tracking-[1px] text-[#7A8E9B] mb-1.5">Telefone</label>
        ...
        <div>
          <label className="block text-[1.125rem] uppercase tracking-[1px] text-[#7A8E9B] mb-1.5">Notas</label>
        ...
        <div>
          <label className="block text-[1.125rem] uppercase tracking-[1px] text-[#7A8E9B] mb-1.5">Tags</label>
          <div className="flex flex-wrap gap-1.5 mb-2">
            {form.tags.map((t) => (
              <span key={t} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-[#003E51]/[0.06] text-[#003E51] uppercase text-[1.125rem]">
                {t}
                <button onClick={() => removeTag(t)} className="text-[#003E51]/40 hover:text-red-500 ml-0.5">&times;</button>
              </span>
            ))}
          </div>
        ...
        <div className="flex items-center gap-3 pt-2">
          ...
          <button onClick={onCancel} className="px-6 py-2.5 rounded-xl text-[1.125rem] text-[#7A8E9B] hover:text-[#003E51] transition-colors">Cancelar</button>
        </div>
      </div>
    </div>
  );
